...
vec3 CalculateLight(vec3 albedo, vec3 radiance, vec3 N, vec3 V, vec3 L, vec3 F0, float roughness, float metallic)
{
        ...
        vec3 F    = fresnelSchlick(min(max(dot(H, V), 0.0),1.0), F0);       
        ...
        float denominator = 4.0 * min(max(dot(N, V), 0.0),1.0) * min(max(dot(N, L), 0.0),1.0);
...
vec3 CalculateLightNoFresnel(vec3 albedo, vec3 radiance, vec3 N, vec3 V, vec3 L, vec3 F0, float roughness, float metallic)
{
        vec3 H = normalize(V + L);
        // cook-torrance brdf
        float NDF = DistributionGGX(N, H, roughness);        
...
void mainaaa()
{
    vec3 WorldPos = texture(positionMap,i.texCoords).rgb;
...
void mainAha()
{
    ...
    vec3 Lo = vec3(0.0);
        ...
        float NDF = DistributionGGX(N, H, roughness);        
        float G   = GeometrySmith(N, V, L, roughness); 
        float cosTheta = min(max(dot(H, V), 0.0),1.0);
...
{             
    ...
    for(int i = 0; i < 4; ++i)
    {
        ...
        vec3 diffuse = max(dot(N, lightDir), 0.0) * albedo * lightColors[i]/100;
        lighting += diffuse;
    }
...
{		
    ...
    Lo+=CalculateLightNoFresnel(albedo, dirLightColor, N, V, dirLight, F0, roughness, metallic)*(1.0 - shadow);
